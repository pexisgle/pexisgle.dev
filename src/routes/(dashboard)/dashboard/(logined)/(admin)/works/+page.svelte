<script lang="ts">
	...
	function exportWorks() {
		// Clean data for export (remove implementation details if any, though schema output is reliable)
		const cleanData = $state.snapshot(data.works).map((w) => ({
			title: w.title,
			description: w.description,
			type: w.type,
			creationPeriod: w.creationPeriod,
			article: w.article,
			urls: w.urls.map((u) => ({ title: u.title, url: u.url }))
		}));
	...
	function exportWorkItem(work: (typeof data.works)[number]) {
		const cleanData = {
			title: work.title,
		...
		link.href = url;
		// Sanitize filename
		const filename = work.title.replace(/[^a-z0-9]/gi, '_').toLowerCase();
		link.download = `work-${filename}.json`;
		document.body.appendChild(link);
		link.click();
		document.body.removeChild(link);
		URL.revokeObjectURL(url);
...
<div class="px-4 pt-6">
	...
	<div class="mb-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
		...
		<div class="flex flex-wrap gap-2">
			<Button color="alternative" size="sm" onclick={exportWorks} class="gap-2">
				...
				<TableBodyRow>
					...
					<TableBodyCell>
						<div class="flex gap-2">
							<Button size="xs" color="alternative" onclick={() => exportWorkItem(work)}>
								<DownloadOutline class="h-4 w-4" />
							</Button>
							<Button href="/dashboard/works/{work.id}/edit" size="xs" color="light">
...
<!-- Import All Modal -->
<Modal bind:open={importModalOpen} size="sm" title="Import All Works" autoclose={false}>
	<form
		method="POST"
		action="?/import"
	...
	>
		<div>
			...
			/>
			<Helper class="mt-2"
				>Select a JSON file containing works to replace existing data. <span class="text-red-500"
					>Warning: This will replace ALL existing works.</span
				></Helper
			>
		</div>
...
<!-- Import Item Modal -->
<Modal bind:open={importItemModalOpen} size="sm" title="Import Work Item" autoclose={false}>
	<form
		method="POST"
		action="?/importItem"
		enctype="multipart/form-data"
	...
	>
		<div>
			...
			<Fileupload
				...
				required
			/>
			<Helper class="mt-2">Select a JSON file containing a single work item to add.</Helper>
		</div>
		<div class="flex justify-end gap-2">
