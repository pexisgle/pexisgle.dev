...
import { award } from '$lib/server/db/schema';
...
import { awardFormSchema, deleteFormSchema, importFormSchema } from '$lib/server/form-schemas';
import { awardListSchema, reorderDataSchema } from '$lib/server/data-schemas';
...
export const load: PageServerLoad = async ({ platform, parent }) => {
	...
	const awards = await db.select().from(award).orderBy(asc(award.order));

	// Provide empty validated forms for each action type
	const form = await superValidate(arktype(awardFormSchema));
	...
	return { awards, user, form, deleteForm, importForm };
};
...
export const actions: Actions = {
	create: async ({ request, platform }) => {
		...
		const form = await superValidate(request, arktype(awardFormSchema));
		if (!form.valid) {
			return fail(400, { form });
		}

		const { id: submittedId, name, date, status, order } = form.data;
		const statusValue =
			status && ['Gold', 'Silver', 'Bronze'].includes(status)
				? (status as 'Gold' | 'Silver' | 'Bronze')
				: null;

		const { db: d1 } = requirePlatformForActions(platform);
		const db = generateDB(d1);
		const awardId = submittedId || uuidv4();
		...
		return {
			form,
			success: true,
			award: { id: awardId, name, date: date ?? null, status: statusValue, order }
		};
	},
	...
	update: async ({ request, platform }) => {
		...
		const { id, name, date, status, order } = form.data;
		if (!id) {
		...
		return {
			form,
			success: true,
			award: { id, name, date: date ?? null, status: statusValue, order }
		};
	},
	...
	reorder: async ({ request, platform }) => {
		...
		try {
			...
			for (const item of data) {
				batch.push(db.update(award).set({ order: item.order }).where(eq(award.id, item.id)));
			}
	...
	import: async ({ request, platform }) => {
		...
		try {
			...
			const result = awardListSchema(json);
