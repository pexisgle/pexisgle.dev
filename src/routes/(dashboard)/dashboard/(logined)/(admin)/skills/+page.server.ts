...
import { skill } from '$lib/server/db/schema';
...
import { skillFormSchema, deleteFormSchema, importFormSchema } from '$lib/server/form-schemas';
import { skillListSchema, reorderDataSchema } from '$lib/server/data-schemas';
...
export const load: PageServerLoad = async ({ platform, parent }) => {
	...
	const skills = await db.select().from(skill).orderBy(asc(skill.order));

	const form = await superValidate(arktype(skillFormSchema));
	...
	return { skills, user, form, deleteForm, importForm };
};
...
export const actions: Actions = {
	create: async ({ request, platform }) => {
		...
		const form = await superValidate(request, arktype(skillFormSchema));
		if (!form.valid) {
			return fail(400, { form });
		}

		const { id: submittedId, name, icon, confidence, order } = form.data;
		const skillId = submittedId || uuidv4();
		...
		return { form, success: true, skill: { id: skillId, name, icon, confidence, order } };
	},
	...
	update: async ({ request, platform }) => {
		...
		const { id, name, icon, confidence, order } = form.data;
		if (!id) {
		...
		return { form, success: true, skill: { id, name, icon, confidence, order } };
	},
	...
	reorder: async ({ request, platform }) => {
		...
		try {
			...
			for (const item of data) {
				batch.push(db.update(skill).set({ order: item.order }).where(eq(skill.id, item.id)));
			}
	...
	import: async ({ request, platform }) => {
		...
		try {
			...
			const result = skillListSchema(json);
