<script lang="ts">
	...
	let localCertifications = $derived(
		reorderedCertifications ?? data.certifications.slice().sort((a, b) => a.order - b.order)
	);
	...
	const {
		...
		onResult: ({ result }) => {
			if (result.type === 'failure') {
				...
				toast.success(editingId === null ? '資格を作成しました' : '資格を更新しました');
				const payload = (result.data ?? result) as { certification?: Certification };
				...
				if (certification) {
					if (editingId === null) {
						...
						form.set({
							...
							date: undefined,
							status: undefined,
							order: Math.max(...localCertifications.map((c) => c.order), 0) + 1
						});
					} else {
	...
	function startCreate() {
		...
		form.set({
			...
			date: undefined,
			status: undefined,
			order: Math.max(...localCertifications.map((c) => c.order), -1) + 1
		});
	}
	...
	function exportJSON() {
		const cleanData = $state.snapshot(localCertifications).map(({ name, date, status, order }) => ({
			name,
			date,
			status,
			order
	...
	function getReorderedItemsJSON() {
		return JSON.stringify(
			localCertifications.map((item, index) => ({
				id: item.id,
				order: index
			}))
		);
					...
					use:enhance={() => {
						return async ({ result }) => {
							if (result.type === 'success') {
								...
								isReordering = false;
								await window.location.reload(); // Reload to get fresh data
							} else {
			...
			<Card class="h-fit p-6">
				...
				>
					...
					<div>
						...
						<Input id="name" name="name" placeholder="資格名" bind:value={$form.name} required />
						{#if $errors.name}
