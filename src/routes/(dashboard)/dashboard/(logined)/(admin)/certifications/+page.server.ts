...
import { certification } from '$lib/server/db/schema';
...
import { certificationListSchema, reorderDataSchema } from '$lib/server/data-schemas';
...
export const load: PageServerLoad = async ({ platform, parent }) => {
	...
	const certifications = await db.select().from(certification).orderBy(asc(certification.order));
...
export const actions: Actions = {
	create: async ({ request, platform }) => {
		...
		const db = generateDB(d1);
		const certId = submittedId || uuidv4();
		...
		return {
			form,
			success: true,
			certification: { id: certId, name, date: date ?? null, status: statusValue, order }
		};
	},
	...
	update: async ({ request, platform }) => {
		...
		return {
			form,
			success: true,
			certification: { id, name, date: date ?? null, status: status ?? null, order }
		};
	},
	...
	reorder: async ({ request, platform }) => {
		...
		try {
			...
			for (const item of data) {
				batch.push(
					db.update(certification).set({ order: item.order }).where(eq(certification.id, item.id))
				);
			}
	...
	import: async ({ request, platform }) => {
		...
		try {
			...
			const result = certificationListSchema(json);
			...
			const batch: BatchItem<'sqlite'>[] = [];
			batch.push(db.delete(certification));
			...
			if (data.length > 0) {
				...
				for (const chunk of chunks) {
					batch.push(
						db.insert(certification).values(
							chunk.map((item) => ({
								id: uuidv4(),
								name: item.name,
								date: item.date ?? null,
